...
- **Model mapping (`toModels()`)** — converts OmniRoute `/v1/models` rows into pi model entries:
  ...
  `thinkingLevelMap` is stamped from the gateway's `capabilities.effort_tiers`,
  falling back to pi's built-in provider catalogs (alias prefix stripped) when the
  gateway publishes none — never send an invalid `reasoning_effort` like `high` on
  qwen3.8-max. If neither source has the info, no map is written and pi's default
  `off`/`minimal`/`low`/`medium`/`high` applies.
